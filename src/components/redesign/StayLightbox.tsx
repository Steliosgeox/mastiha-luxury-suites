...
export default function StayLightbox({ index, locale, onClose }: { index: number; locale: StayLocale; onClose: () => void }) {
  ...
  return <Lightbox open index={index} close={onClose} slides={stayPhotos.map((photo) => ({ src: photo.src, width: photo.width, height: photo.height, alt: `${c.captions[photo.id]} · Mastiha Luxury Suites`, title: c.captions[photo.id] }))}
    plugins={[Captions, Counter, Fullscreen, Thumbnails, Zoom]} carousel={{ preload: 1 }} controller={{ closeOnBackdropClick: true }} thumbnails={{ width: 80, height: 54, gap: 8 }}
